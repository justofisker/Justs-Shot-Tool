...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion && grab != Grab.NONE:
		...
		if grab == Grab.X || grab == Grab.XY:
			get_parent().global_position.x = snappedf(pos.x - grab_offset.x, 8) / 8
		if grab == Grab.Y || grab == Grab.XY:
			get_parent().global_position.y = snappedf(pos.y - grab_offset.y, 8) / 8
